...
import { Search, Download, BookOpen, Star, RefreshCw, FileText, X, Mic, Volume2, Eye, EyeOff, Highlighter, ListPlus, BrainCircuit, ListMusic, Sparkles, Loader2, Layers } from 'lucide-react';
...
import { generatePhysicsAnswer, AIEngine } from './lib/ai';
...
const InteractiveCard: React.FC<{ question: Question; onPrint: (q: Question) => void }> = ({ question, onPrint }) => {
  ...
  const handleGenerateAiAnswer = async () => {
    ...
    const answer = await generatePhysicsAnswer(question.title, selectedEngine);
    setAiAnswer(answer);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
        <h3 className="text-lg sm:text-xl font-bold text-slate-800 leading-snug flex-1 pr-24 relative">
          ...
          <div className="absolute top-0 right-0 flex items-center gap-1">
            ...
            <button
              onClick={() => {
                // We will dispatch a custom event to open the flashcard creator
                window.dispatchEvent(new CustomEvent('open-flashcard-creator', { detail: question }));
              }}
        ...
        {showAnswer && (
          ...
          >
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Key Points to Cover:</p>
            ...
            <div className="bg-indigo-50/50 rounded-2xl p-5 border border-indigo-100">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-bold text-indigo-900">AI Detailed Explanation</span>
                </div>
                <div className="flex items-center space-x-2 bg-white rounded-lg p-1 border border-indigo-100">
                  {(['gemini', 'chatgpt', 'claude'] as AIEngine[]).map((engine) => (
                    <button
                      key={engine}
              ...
              {aiAnswer ? (
                <div className="prose prose-sm prose-indigo max-w-none">
                  ...
                  <button 
                    ...
                    className="mt-4 text-xs font-bold text-indigo-600 hover:text-indigo-700 flex items-center"
                  >
                    {isGeneratingAi ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <RefreshCw className="w-3 h-3 mr-1" />}
                    Regenerate with {selectedEngine}
                  </button>
                </div>
              ) : (
                <button
                  onClick={handleGenerateAiAnswer}
                  disabled={isGeneratingAi}
                  className="w-full py-3 px-4 bg-white border border-indigo-200 rounded-xl text-sm font-bold text-indigo-600 hover:bg-indigo-50 transition-colors flex items-center justify-center disabled:opacity-50"
                >
                  {isGeneratingAi ? (
                    <>
                    ...
                    <>
                      ...
                      Generate Answer with {selectedEngine}
                    </>
                  )}
                </button>
              )}
...
export default function App() {
  ...
  const [flashcardCreatorInitialQuestion, setFlashcardCreatorInitialQuestion] = useState<Question | null>(null);

  useEffect(() => {
    const handleOpenFlashcardCreator = (e: Event) => {
      const customEvent = e as CustomEvent<Question>;
      setFlashcardCreatorInitialQuestion(customEvent.detail);
      setActiveTab('flashcards');
    };
    window.addEventListener('open-flashcard-creator', handleOpenFlashcardCreator);
    return () => window.removeEventListener('open-flashcard-creator', handleOpenFlashcardCreator);
  }, []);
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-violet-200 selection:text-violet-900 pb-24">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'flashcards' && (
          <FlashcardMode 
            initialQuestion={flashcardCreatorInitialQuestion} 
            onCloseCreator={() => setFlashcardCreatorInitialQuestion(null)} 
          />
        )}
